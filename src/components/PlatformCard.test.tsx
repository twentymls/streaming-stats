...
describe("PlatformCard", () => {
  it("renders platform name and formatted main stat", () => {
    render(<PlatformCard source="spotify" stats={{ streams: 1500000 }} />);
    expect(screen.getByText("Spotify")).toBeInTheDocument();
    expect(screen.getByText("1.5M")).toBeInTheDocument();
  });

  it("picks streams as main stat over followers", () => {
    render(<PlatformCard source="spotify" stats={{ followers: 500, streams: 1000 }} />);
    // Main stat should be streams (1.0K), followers as sub-stat
    expect(screen.getByText("1.0K")).toBeInTheDocument();
  });

  it("picks views as main stat", () => {
    render(<PlatformCard source="youtube" stats={{ views: 2000, followers: 100 }} />);
  ...
  it("calls onClick when clicked", () => {
    ...
    render(<PlatformCard source="spotify" stats={{ streams: 1000 }} onClick={handleClick} />);
    fireEvent.click(screen.getByText("Spotify"));
    expect(handleClick).toHaveBeenCalledOnce();
  });

  it("handles unknown source gracefully", () => {
    render(<PlatformCard source="pandora" stats={{ plays: 500 }} />);
